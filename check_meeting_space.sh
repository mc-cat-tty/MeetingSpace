...
# Launch this script to check if given room_code is valid or not
# Notice that meeting rooms are global and any valid authorization parameters (regardless of account) are suitable
# Usage: ./meetIsActive.sh SAPISIDHASH x-goog-api-key x-goog-authuser __Secure-3PSID __Secure-3PAPISID room_code
# Output: POST request HTTP status code
...
if [[ "$1" == "-h" ]] || [[ "$1" == "--help" ]]
then
    echo "Usage: ./meetIsActive.sh SAPISIDHASH x-goog-api-key x-goog-authuser __Secure-3PSID __Secure-3PAPISID room_code"
    exit 0
fi
